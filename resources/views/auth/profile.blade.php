...
@section('title') BCT Coin Profile Page  @endsection
@section('style')
<style type="text/css">
    .secret{
        color:#000;
  ...
  <div class="row">
    <div class="col-sm-12">
      ...
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a @if($slug == "2") href="{{ url('user/profile')}}" @else href="{{ url('admin/profile')}}" @endif>Profile</a>
        </li>
      </ol>
    </div>
    <div class="col-md-4">
      <div class="card">
        <div class="card-body">
          ...
          <form  enctype="multipart/form-data" action="{{ url('admin/profilePicture')}}/{{Sentinel::getUser()->id}}" method="post">
            @elseif($slug == '4')
              <form  enctype="multipart/form-data" action="{{ url('support/profilePicture')}}/{{Sentinel::getUser()->id}}" method="post">
          @else
              ...
              <div class="profile text-center">
                <div class="p-relative">
                  ...
                  <div class="image-upload">
                    ...
                    <input id="file-input" type="file" name="photo" accept="image/x-png,image/gif,image/jpeg" onchange="readURL(this,0)" />
                  </div>
                </div>
                <div>
                  <button type="submit" class="default-btn mt-4">Update Profile</button>
                </div>

                <!-- <img src="../assets/images/avtar.png" />-->
                <h3 class="mt-3 mb-4 blue-font">{{ Sentinel::getUser()->first_name }}</h3>
                <h4>{{ Sentinel::getUser()->last_name }}</h4>
          ...
          <div class="tab-pane fade @if(!session('validator'))  show active @endif" id="profile" role="tabpanel" aria-labelledby="profile-tab">
            <div class="col-md-12">
              ...
              <form class="form-horizontal theme-form mt-5 row"  action="{{ url('admin/profile')}}/{{Sentinel::getUser()->id}}" method="post">
                @elseif($slug == '4')
                  <form class="form-horizontal theme-form mt-5 row"  action="{{ url('support/profile')}}/{{Sentinel::getUser()->id}}" method="post">
                    @else
                <form class="form-horizontal theme-form mt-5 row"  action="{{ url('user/profile')}}/{{Sentinel::getUser()->id}}" method="post">
                  ...
                  <div class="form-group col-md-6">
                    ...
                    <input type="text" class="form-control" name="first_name" value="{{ Sentinel::getUser()->first_name }}" id="first_name" autocomplete="off">
                    @if ($errors->has('first_name'))
                    <span class="help-block text-danger">
                  ...
                  <div class="form-group col-md-6">
                    ...
                    <input type="text" class="form-control" name="last_name" value="{{ Sentinel::getUser()->last_name }}" id="last_name" autocomplete="off">
                  ...
                  <div class="form-group col-md-12 text-right">
                    <button type="submit" class="default-btn mt-4" id="profile-update">Update</button>
                  </div>
                </form>
              </div>
            </div>
            <div class="tab-pane fade mt-5" id="two-fact" role="tabpanel" aria-labelledby="two-fact-tab">
              ...
              <!--   <div class="col-md-12 text-center">
                ...
                <div class="col-md-6 offset-md-3 text-center">
                  <h5>Once you have scanned the barcode, enter the 6-digit code below from application:</h5>
                  <form class="mt-4 theme-form">
                    <div class="form-group ">
                      <input type="text" class="form-control" id="code" >
                    </div>
                    <div class="form-group">
                      <input type="button" class="btn btn-theme" value="Activate">
                    </div>
                  </form>
                </div> -->
              </div>
            ...
            <div class="tab-pane fade @if(session('validator'))  show active @endif " id="chg-pwd" role="tabpanel" aria-labelledby="chg-pwd-tab">
              <div class="col-md-12">
                ...
                <form class="form-vertical theme-form mt-5 row col-md-12" action="{{url('admin/changePassword')}}/{{Sentinel::getUser()->id}}" method="post">
                 ...
                 @elseif($slug == '4')
                      <form class="form-vertical theme-form mt-5 row col-md-12" action="{{url('support/changePassword')}}/{{Sentinel::getUser()->id}}" method="post">
                  @else
                  <form class="form-vertical theme-form mt-5 row col-md-12" action="{{url('user/changePassword')}}/{{Sentinel::getUser()->id}}" method="post">
                    ...
                    <div class="form-group col-md-12 text-right">
                      <button type="submit" class="default-btn mt-4">Change</button>
                    </div>
                  </form>
                </div>
              </div>
...
<!--
                            ...
                            <div class="text-center">
                              <code>If enabled then each time you login or make a withdrawal at BCT you will be prompted to complete the 2-Step Verification which can only be finished by having access to your smartphone.</code>
                            </div>


                          </div>
                          <div class="modal-footer text-center">
                            <button type="button" class="default-btn mt-4" data-dismiss="modal">Close</button>
                            <button type="submit" class="default-btn mt-4" id="enable-2fa" >Yes Enable Google 2FA Authenticator</button>
                          </div>
                        </form>
                      </div><!-- /.modal-content -->
                    </div><!-- /.modal-dialog -->
