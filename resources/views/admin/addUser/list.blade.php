...
<div class="dashboard-body">
  <div class="row">
    <div class="col-sm-12">
      <h4 class="page-title">Manage Partners / Advisors</h4>
      <ol class="breadcrumb">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                ...
                <a href="{{url('add-token/')}}/{{$user->user->id}}" class="btn btn-success btn-sm">Send Token</a>
                @if($user->user->status == 1)
                <button onclick="userStatus({{$user->user->id}},'0')"  class="btn btn-warning btn-sm">Block</button>
                @elseif($user->user->status == 0)
                <button  onclick="userStatus({{$user->user->id}},'1')" class="btn btn-success btn-sm">Active</button>
                @elseif($user->user->status == 2)
                <button  onclick="userStatus({{$user->user->id}},'1')" class="btn btn-success btn-sm">Active</button>
                @endif
...
function userStatus(arg1, arg2) {
    ...
    if (arg2 == '0') { // user status is 0
        ...
        swal({
                ...
                text: "You Really Sure Of successsuccess !",
                type: "warning",
                showCancelButton: true,
                confirmButtonColor: "#DD6B55",
